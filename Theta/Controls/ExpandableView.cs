...
namespace Theta.Controls
{
    public class ExpandableView : StackLayout
    {
        ...
        public static readonly BindableProperty StatusProperty = BindableProperty.Create(nameof(Status), typeof(enExpandStatus), typeof(ExpandableView), default(enExpandStatus), BindingMode.OneWayToSource);
        ...
        public enExpandStatus Status
        {
            get => (enExpandStatus)GetValue(StatusProperty);
        ...
        {
            RaiseStatusChanged(IsExpanded ? enExpandStatus.Expanding : enExpandStatus.Collapsing);

            if (_shouldIgnoreAnimation)
            {
                RaiseStatusChanged(IsExpanded ? enExpandStatus.Expanded : enExpandStatus.Collapsed);
            ...
            new Animation(v => SecondaryView.HeightRequest = v, _startHeight, _endHeight)
                .Commit(SecondaryView, ExpandAnimationName, 16, length, easing, (value, interrupted) =>
                {
                    ...
                    if (!IsExpanded)
                    {
                        ...
                        RaiseStatusChanged(enExpandStatus.Collapsed);
                        return;
                    }
                    RaiseStatusChanged(enExpandStatus.Expanded);
                });
        }

        private void RaiseStatusChanged(enExpandStatus status)
        {
            Status = status;
    ...
    {
        public StatusChangedEventArgs(enExpandStatus status)
        {
            Status = status;
        }

        public enExpandStatus Status { get; }
    }

    public enum enExpandStatus
    {
        Expanding,
        Expanded,
        Collapsing,
